  ...
  API backend untuk aplikasi Todo List, dibangun sebagai bollerplate belajar backend Node.js: struktur folder Controller-Service-Model, middleware, autentikasi JWT, proteksi API Key, validasi input, pagination, automated testing, dan dokumentasi Swagger. 
  ...
  - CRUD Todo dengan struktur Controllers-Service-Model
  ...
  - Automated test dengan Jest dan Superjest
  ...
  ```bash
  git clone https://github.com/Rayyara/todo-api.git
  cd todo-api
  npm install
  ```
  ...
  | `JWT_EXPIRES_IN`   | Ya    | `id`                                    | Masa berlaku token JWT                                        |
  ...
  Test memakai `mongo-memory-server`, jadi tidak akan menyentuh database development sungguhan.
